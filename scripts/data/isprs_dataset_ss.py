...
class ISPRSRefDataset_SS(Dataset):
    """
    ...
    def __init__(
        ...
        if unlabeled_mask_path is not None:
            ...
            for mask_file in sorted(unlabeled_mask_path.glob("*.tif")):
                ...
                self.samples.append((str(img_path), pseudo_mask_path, query_text))
                pseudo_count += 1
                # print("samples", self.samples[-1])  # Debug: print the last sample added
                # print("length samples", len(self.samples))  # Debug: print total number of samples after each addition
...
if __name__ == "__main__":
    ...
    ds = ISPRSRefDataset_SS(data_dir=data_root, split="train", variant="standard")
    ...
    data_root = "/home/icclab/Documents/lqw/DatasetMMF/VaihingenRef/"
    ds = ISPRSRefDataset_SS(data_dir=data_root, 
