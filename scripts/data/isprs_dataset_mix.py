...
class ISPRSRefDataset_mix(Dataset):
    """
    Vaihingen Referring Segmentation Dataset wrapped for SAM3 training.
    ...
    Args:
        data_dir: Root directory of VaihingenRef dataset
                  (e.g., /home/icclab/Documents/lqw/DatasetMMF/VaihingenRef/)
        split: 'train', 'valid', or 'test'
        variant: 'standard' or 'complex' (text description variant)
        resolution: Target image resolution (default: 1008 for SAM3)
    """

    MIX_VERSIONS = ['simple', 'standard', 'complex']
    ...
    def __init__(
        ...
        # ── Mixed-version training: load all four text variants, align by mask filename ──
        if variant == "mix":
            # version_sentences:  mask_name -> [sent_simple, sent_standard, sent_complex]
            ...
            # self.samples: List of (img_path, mask_path, [sent_simple, sent_standard, sent_complex])
            self.samples = []
            for mask_name in sorted(sample_meta.keys()):
                img_path, mask_path = sample_meta[mask_name]
                self.samples.append((img_path, mask_path, version_sentences[mask_name]))

            print(
                f"VaiRefDataset [{split}] loaded: {len(self.samples)} samples"
                f" (variant=mix, {len(self.MIX_VERSIONS)} versions/sample)"
            )

        else:
            # ── Single-variant loading (original behaviour) ──
    ...
    def __getitem__(self, idx: int) -> Datapoint:
        ...
        if self.variant == "mix":
            # query_text is a list of sentences; randomly pick one
            choice_idx = np.random.randint(0, len(query_text))
            query_text = query_text[choice_idx]
...
if __name__ == "__main__":
    ...
    ds = ISPRSRefDataset_mix(data_dir=data_root, split="train", variant="standard")
    ...
    print("\n✅ VaiRefDataset working correctly!")
